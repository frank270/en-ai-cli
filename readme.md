...
- ✅ **安全執行**：所有指令執行前都需要你的確認，針對危險指令有分級警告與 `YES` 強制確認機制
- 🛡️ **路徑安全性**：自動分析指令路徑，防止誤刪父目錄 (`..`) 或系統關鍵路徑
- 📂 **目錄追蹤**：內建 `cd` 攔截，讓 AI 在對話中切換的目錄能持續生效
- 🎭 **角色系統**：支援多種 AI 角色（指令專家、程式碼審查、親切助手），可快速切換 Persona
...
### 📅 第五階段：斜線指令與豐富化
- [ ] 斜線指令 (Slash Commands) 整合 (`/help`, `/role`)
- [ ] 多語言支援介面
- [ ] 網路連線與逾時最佳化
